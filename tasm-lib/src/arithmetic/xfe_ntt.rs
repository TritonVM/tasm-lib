...
impl BasicSnippet for XfeNtt {
    ...
    fn code(
        ...
        let tasm_list_unsafe_u32_length_long_xfe =
            library.import(Box::new(UnsafeLength(DataType::XFE)));
        let tasm_list_unsafe_u32_new_xfe = library.import(Box::new(UnsafeNew(DataType::XFE)));
        let tasm_list_unsafe_u32_push_xfe = library.import(Box::new(UnsafePush(DataType::XFE)));
        let tasm_arithmetic_u32_safe_add_u32 = library.import(Box::new(SafeAdd));
        let tasm_list_unsafe_u32_push_bfe = library.import(Box::new(UnsafePush(DataType::BFE)));
        let tasm_list_unsafe_u32_new_bfe = library.import(Box::new(UnsafeNew(DataType::BFE)));
        triton_asm! {
            {entrypoint}:
        dup 1
        call {tasm_list_unsafe_u32_length_long_xfe}
        dup 0
        push 2
        eq
        push 1
        ...
        {base_case}:
        push 2
        call {tasm_list_unsafe_u32_new_xfe}
        dup 0
        dup 3
        push 0
        push 3
        ...
        swap 3
        pop
        call {tasm_list_unsafe_u32_push_xfe}
        dup 0
        dup 3
        push 0
        push 3
        ...
        dup 2
        dup 1
        call {tasm_list_unsafe_u32_push_bfe}
        dup 0
        dup 10
        mul
        swap 1
        ...
        call {entrypoint}
        dup 5
        call {tasm_list_unsafe_u32_new_bfe}
        push 0
        push 1
