...
impl FriVerify {
    ...
    fn get_colinearity_check_x(&self, idx: u32, round: usize) -> XFieldElement {
        let domain_value = self.domain.offset * self.domain.generator.mod_pow_u32(idx);
    ...
    fn inner_verify(
        ...
        if last_poly_degree > max_degree_of_last_round as isize {
            println!(
        ...
        let mut a_indices =
            proof_stream.sample_indices(self.domain.length as u32, self.colinearity_checks_count);
        ...
        let mut current_domain_len = self.domain.length as u32;
        ...
        for r in 0..num_rounds {
            ...
            debug_assert_eq!(self.colinearity_checks_count, a_indices.len() as u32);
            debug_assert_eq!(self.colinearity_checks_count, b_indices.len() as u32);
            debug_assert_eq!(self.colinearity_checks_count, a_values.len() as u32);
            debug_assert_eq!(self.colinearity_checks_count, b_values.len() as u32);
            ...
            let c_values = (0..self.colinearity_checks_count as usize)
                .map(|i| {
        ...
        if !(0..self.colinearity_checks_count as usize)
    ...
    pub fn pseudorandom_fri_proof_stream(&self, seed: [u8; 32]) -> VmProofStream {
        ...
        let mut codeword = [
            coefficients,
            vec![XFieldElement::zero(); self.domain.length - max_degree - 1],
        ]
        .concat();
        let n = codeword.len();
        ntt::<XFieldElement>(
        ...
        let fri = Fri::new(
            ...
            self.colinearity_checks_count as usize,
        );
