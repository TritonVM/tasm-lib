...
/// ### Preconditions
///
/// - a non-deterministic digest is available
...
mod tests {
    ...
    impl MerkleStepU64Index {
        fn set_up_initial_state(&self, leaf_index: u64) -> ProcedureInitialState {
            ...
            ProcedureInitialState {
                stack,
                nondeterminism: NonDeterminism::default().with_digests(vec![random()]),
                public_input: vec![],
                sponge: None,
            }
        }
    ...
    impl Procedure for MerkleStepU64Index {
        fn rust_shadow(
            ...
            let (left_digest, right_digest) = if stack_digest_is_left_sibling {
                (stack_digest, nondeterminism.digests[0])
            } else {
                (nondeterminism.digests[0], stack_digest)
            };
    ...
    fn unit_test() {
        ...
        fn assert_expected_behavior(mt_index: u64, expected_parent_index: u64) {
            ...
            let final_state = crate::test_helpers::tasm_final_state(
                &ShadowedProcedure::new(MerkleStepU64Index),
                &initial_state.stack,
